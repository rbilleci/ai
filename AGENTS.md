...
### Computed Provenance
`STD-FIGURE-PROVENANCE`: Derive every figure by computation from a retained artifact, at writing time, and cite the URL, query, command, or file path that produced it. Never transcribe a figure from memory, an earlier conversation, or an earlier draft. When no artifact exists, retain the derivation beside the evidence or delete the sentence. In a chat reply, the session transcript is the retained artifact.
...
### Absolute References
`STD-ABSOLUTE-REFERENCES`: Write for a reader who cannot see the writing date or the surrounding page. Never use "recently" or "next week." Use exact dates like "October 24, 2024." Never write "See the configuration above"; moving content breaks every positional reference to it. Cite per `STD-CROSS-REFERENCES`.
...
### Author Anonymity
Omit author names, titles, credentials, and "Written By" metadata; a document stands on the strength of its data and logic.
...
`STD-IMMUTABLE-IDENTIFIERS`: Track discrete requirements using immutable identifiers of the form `PREFIX-SEMANTIC-SLUG`: the prefix names the governing document, and the slug names the requirement subject. Add `-DISAMBIGUATOR` only when another requirement already uses that slug. Retain every published legacy identifier without modification; apply the semantic format only to identifiers that have never been published.
